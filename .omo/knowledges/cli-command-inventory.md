...
- Departure: `space leave <SPACE_REF> [--json]` asks the Space authority to sign the next manifest generation removing this Endpoint; an unreachable authority fails without local change, and the owner of a Space cannot leave it. The authority answers with the bounded `EnrollmentPage` stream, so a long history still fits the transport. `space_left` carries only `space_id` and `name`, never a stale pre-departure member count.
...
- Member labels never carry the Space name: creation and enrollment both label a member `endpoint-<first 8 identity bytes>`, and nothing writes the placeholder `local-endpoint`.
