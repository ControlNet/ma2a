...
necessary, and waits until the endpoint is live. Concurrent starters converge through a stable
startup lock before the daemon takes its separate singleton lock. `shutdown` waits until the
...
the namespaced pipe uses a protected DACL granting access only to the owner and Local System.
