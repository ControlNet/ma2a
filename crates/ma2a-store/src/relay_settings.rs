...
impl Repository {
    ...
    pub fn relay_configuration(&self) -> Result<RelayConfiguration, StoreError> {
        ...
        let mut public_urls = self
            .connection
            .prepare("SELECT relay_url FROM relay_public_fallback_urls ORDER BY position")?;
        let public_relay_urls = public_urls
            .query_map([], |row| row.get::<_, String>(0))?
            .collect::<Result<Vec<_>, _>>()?;
        let mut served = self
            .connection
            .prepare("SELECT space_id FROM relay_served_spaces ORDER BY space_id")?;
